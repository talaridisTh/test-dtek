        ...
        <th width="100%" style="font-size: 11px !important;">
          <table style="width: 100%; vertical-align: bottom;">
            ...
            @if ($store_info)
              <tr>
                <td style="width:210px; border: 0px; padding-left:15px;overflow: hidden;">
                  <img style="margin:-50px 0;" width="170" src="{{ asset('assets/media/logos/logo.png') }}" alt="{{ $store_info['name'] }}" class="img-circle"/>
                </td>
            ...
            <tr>
              ...
              <td style="border: 0px;padding-left:30px; color:#444">
                    <div style="padding-left: 20px;font: 10px lighter;"> <strong>Αρ.Παραγγελίας:</strong> Ν&deg;  ΠΑΡ-{{ $order['order_id'] }}</div>
                    ...
                    <div style="padding-left: 20px;font: 10px lighter;"> <strong>Τρόπος πληρωμής:</strong>
          ...
          <table class="table" style="font: 12px lighter; width:100%" border="0" cellspacing="0">
            <tr class="info" style="background-color: #E0E5E8;">
                <th align="center">#</th>
                <th class="description" width="30%" align="left">Προϊόν</th>
                <th style="text-align:center;">Ποσότητα</th>
                <th style="font-weight:normal;height:30px;border:none;width:80px;">Τιμή<br/>Μονάδας</th>
                <th style="font-weight:normal;height:30px;border:none;text-align:left;width:80px;">Τιμή Καταλόγου</th>
                <th style="font-weight:normal;height:30px;border:none;text-align:left;">Έκπτωση <br> % | Ποσό</th>
                <th style="font-weight:normal;height:30px;border:none;text-align:right;padding-right:10px;">Τελική Τιμή</th>
            </tr>
              @php
                  $total_tax = 0;
                  $subtotal = 0;
              ...
              @foreach ($products as $p)
                  ...
                  <tr nobr="true">
                      ...
                      <span style="font-size:12px;"><strong>{{ $p['manufacturer_name'] }} {{ $p['details']['name'] }}  {{ $p['details']['width'] }}/{{ $p['details']['height_percentage'] }}{{ $p['details']['radial_structure'] }}{{ $p['details']['diameter'] }} {{ $p['details']['weight_flag'] }}{{ $p['details']['speed_flag'] }} @if($p['details']['fitting_postition'] == 1) E @elseif($p['details']['fitting_postition'] == 2) Π @else Ε/Π @endif @if($p['details']['tube_type'] == 0) TL @elseif($p['details']['tube_type'] == 1) TT @else TL/TT @endif {{ $p['details']['model'] }}</strong></span>
                      </td>
                      <td style="height:30px;text-align:center">{{ $p['quantity'] }}</td>
                      <td align="center">{{ $p['price'] }}&euro;</td>
                      <td style="height:30px;text-align:left;" class="amount">
                      {{ round(($p['price'] *$p['quantity']), 2) }}&euro;
                      </td>
              ...
              <tr style="width:100%;height:30px;border-bottom:1px solid #ccc;">
                  ...
                  <td style="height:30px;text-align:center">{{ round($enviromental_tax_count,2) }}</td>
                  <td style="height:30px;text-align:left">{{ round($enviromental_tax/1.24,2) }}&euro;</td>
          ...
          <table style="font: 12px lighter;" border="0" cellspacing="0" align="right">
            <tr>
              ...
              <td class="danger" style="text-align:right;padding:0 10px">{{ number_format($subtotal,2) }} &euro;</td>
            </tr>
            <tr>
              <th class=""><strong>Κόστος Μεταφορικών:</strong></th>
              <td class="danger" style="text-align:right;padding:0 10px;">{{ number_format($order['shipping_cost'],2) }} &euro;</td>
            </tr>
            <tr>
              <th class="" ><strong>Κόστος Πληρωμής:</strong></th>
              <td class="danger" style="text-align:right;padding:0 10px;">{{ number_format($order['payment_cost'],2) }} &euro;</td>
            </tr>
            <tr>
              <th><strong>ΦΠΑ:</strong></th>
              <td class="danger" style="text-align:right;padding:0 10px;">{{ number_format($total_tax,2) }} &euro;</td>
            </tr>
            ...
            @endphp
            <tr>
              <th>Τύπος Έκπτωσης:</th>
              <td class="danger" style="text-align:right;padding:0 10px">
                  {{ $order['discount_type'] == 1 ? 'Ποσοστό' : 'Σταθερό ποσό' }} ({!! $order['discount_type'] == 1 ? $order['discount_amount'].'%' : round($order['discount_amount'],2).'&euro;' !!})
              </td>
            </tr>
            <tr>
              <th>Ποσό Έκπτωσης:</th>
              <td class="danger" style="text-align:right;padding:0 10px">
                  {{ round($total_discount,2) }}&euro;
              </td>
            </tr>
            @endif
            <tr>
              <th>Τελικό Σύνολο:</th>
              <td class="danger" style="text-align:right;padding:0 10px">{{ round($order['order_total'],2) }} &euro;</td>
            </tr>
          </table>
          <br><br><br><br>
